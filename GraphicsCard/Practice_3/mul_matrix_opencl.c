...
int check(const cl_float *a, const cl_float *b, const cl_float *res, int n, int m, int k) {
  ...
  for (int i = 0; i < n; ++i) {
    for (int j = 0; j < k; ++j) {
      if (fabsf(c[i * k + j] - res[i * k + j]) > 10e-9) {
        return 0;
      }
    }
  }
...
int main(int argc, char *argv[]) {
  ...
  clGetDeviceIDs(platforms[1], CL_DEVICE_TYPE_GPU, 1, devices, NULL);
  ...
  FILE *file = fopen("../Practice_3/device.cl", "rb");
...
//  scanf("%zd %zd %zd", &n, &m, &k);
  ...
  cl_float *res = malloc(resMatSize);

  fillRandom(ha, n, m);
  fillRandom(hb, m, k);

  clEnqueueWriteBuffer(queue, a, CL_NON_BLOCKING, 0, fstMatSize, ha, 0, NULL, NULL);
  clEnqueueWriteBuffer(queue, b, CL_NON_BLOCKING, 0, sndMatSize, hb, 0, NULL, NULL);
...
//  size_t work_size[] = {k, n};
  clEnqueueNDRangeKernel(queue, kernel, 2, NULL, work_size, NULL, 0, NULL, &event);
  check_error(event, "Event");

  clEnqueueReadBuffer(queue, c, CL_BLOCKING, 0, resMatSize, res, 0, NULL, NULL);

  FILE *fp = fopen("../Practice_3/out", "w");
  printMatrix(fp, ha, n, m, "A");
  printMatrix(fp, hb, m, k, "B");
  printMatrix(fp, res, n, k, "Res");
/*  if (check(ha, hb, res, n, m, k) == 0) {
  ...
  printf("Work time: %ldms", (fn - st) / 1000);
