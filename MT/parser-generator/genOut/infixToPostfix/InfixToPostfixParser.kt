...
class InfixToPostfixParser(private val lexer: InfixToPostfixLexer) {
  ...
  fun e() : Tree {
    ...
    when (val tokenType = lexer.token.type) {
      TOKEN_TYPE.NUMBER, TOKEN_TYPE.LP -> {
        var tree = t()
        children.add(tree)
        curAttr["t0"] = tree
        
        
        tree = e1(curAttr["t0"]!!.res as String)
        children.add(tree)
        curAttr["e11"] = tree
         res = curAttr["e11"]!!.res as String 
  ...
  fun e1(acc: String) : Tree {
    ...
    when (val tokenType = lexer.token.type) {
      TOKEN_TYPE.PLUS -> {
        ...
        tree = e1(acc + " " + curAttr["t1"]!!.res as String + " +")
        children.add(tree)
        curAttr["e12"] = tree
         res = curAttr["e12"]!!.res as String 
  ...
  fun t() : Tree {
    ...
    when (val tokenType = lexer.token.type) {
      TOKEN_TYPE.NUMBER, TOKEN_TYPE.LP -> {
        ...
        tree = t1(curAttr["f0"]!!.res as String)
        children.add(tree)
        curAttr["t11"] = tree
         res = curAttr["t11"]!!.res as String 
  ...
  fun t1(acc: String) : Tree {
    ...
    when (val tokenType = lexer.token.type) {
      TOKEN_TYPE.MUL -> {
        ...
        tree = t1(acc + " " + curAttr["f1"]!!.res as String + " *")
        children.add(tree)
        curAttr["t12"] = tree
         res = curAttr["t12"]!!.res as String 
