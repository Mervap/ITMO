...
public class UserRepositoryImpl implements UserRepository {
        ...
        try (Connection connection = DATA_SOURCE.getConnection()) {
            try (PreparedStatement statement = connection.prepareStatement(
                ...
                if (statement.executeUpdate() != 1) {
                    throw new RepositoryException("Can't update admin of User.");
                }
            }
        } catch (SQLException e) {
            throw new RepositoryException("Can't update update admin of User.", e);
        }
    }
            ...
            try (PreparedStatement statement = connection.prepareStatement(
                ...
                try (ResultSet resultSet = statement.executeQuery()) {
                    while (resultSet.next()) {
                        users.add(toUserWithoutAdmin(statement.getMetaData(), resultSet));
                    }
                }
            }
        } catch (SQLException e) {
    ...
    public List<User> findAll() {
        ...
        try (Connection connection = DATA_SOURCE.getConnection()) {
            ...
            throw new RepositoryException("Can't find all users with admin information.", e);
        }
        return users;
    }

    @SuppressWarnings("StatementWithEmptyBody")
    private User toUserWithoutAdmin(ResultSetMetaData metaData, ResultSet resultSet) throws SQLException {
        User user = new User();
