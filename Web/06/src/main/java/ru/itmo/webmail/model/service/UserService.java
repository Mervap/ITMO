...
public class UserService {
    ...
    public void validateAdminUpdate(long currentUserId, long userId, boolean admin) throws ValidationException {
        if (!userRepository.find(currentUserId).isAdmin()) {
            throw new ValidationException("You haven't permissions to change this parameter");
        }

        if (userRepository.find(userId).isAdmin() == admin) {
            throw new ValidationException("Article already NotAdmit/Admin");
        }

        if (currentUserId == userId) {
            throw new ValidationException("Do you really want to strip yourself admin?");
        }
    }
    ...
    public List<User> findAllWithAdmin() {
